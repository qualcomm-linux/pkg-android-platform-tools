...
To overlay adbd-usb-gadget, where Device identifiers were observed to change 
across sessions, making device tracking unreliable. 
To ensure consistent and unique identification, update the USB product string to 
include the SoC serial number when available. 
If /sys/devices/soc0/serial_number exists, append its value (in hex format) 
to the hostname and use it as the product string. Otherwise, fall back to the 
default "ADB device" string.
